...
**Statut :** done — implémenté en TDD rouge→vert→refactor (20 slices,
28 tests dans `tests/unit/recommendation.spec.ts` + test de pureté relais).
`npm run lint && npm run typecheck && npm run test` passe (39 tests au total).

- [x] Le module est pur : aucun import Nuxt/HTTP/SQLite/env (vérifié par test
      d'import interdit).
- [x] Formules exactes (CONTEXT.md / spec §5.5) : coût du détour =
      `detourDistanceKm × conso/100 × prix candidat` (D6), économie brute =
      `(prix réf − prix candidat) × quantité`, économie nette = brute − détour.
- [x] `go-to-station` si et seulement si `économie nette ≥ seuil` (strict,
      égalité comprise — CAL-3) ; `recommendedStation` = meilleure `netSavings`.
- [x] `partial-fill` : `X = min(quantité rentabilisant le détour, capacité −
      niveau, preferredQuantity)` (D5) ; si même le plein complet ne rentabilise
      pas → `wait`/`fill-now` selon niveau et tendance.
- [x] Niveau critique (≤ 10 % capacité) → biais `fill-now` même en tendance
      baissière ; niveau élevé → `wait`/`partial-fill`.
- [x] Données fraîcheur : candidate `obsolete` (> 48 h) exclue par défaut ;
      `stale` dégrade la confiance ; écarté si alternative fraîche.
- [x] `isPartial: true` dès qu'une donnée manque (pas de tendance, pas de
      géoloc, prix non résolu) — jamais inféré de la confiance (D1).
- [x] Prix aberrant hors intervalle documenté → `ignoredData`, exclu du calcul
      local ; données incohérentes → recommandation dégradée, jamais d'exception.
- [x] Retour immuable avec **tous** les champs du type `FuelRecommendation`
      remplis (raisons, usedData, ignoredData, calculations, assumptions,
      freshness).
- [x] TDD : les 18 scénarios du §13 sont couverts par des tests Vitest **écrits
      avant** le code de la règle correspondante ; le code minimal fait passer
      chaque test ; refactor puis re-test.
- [x] Formulation de tendance toujours probabiliste (« tendance probable »),
      jamais une certitude (REC-4).
- [x] `npm run lint && npm run typecheck && npm run test` passe (tous les tests
      du §13 verts).

**Note d'implémentation :** pour rester un module 100 % pur, l'input a été
étendu (types `domain/stations/types.ts`) avec `now: Date` (heure courante
injectée, pas de `Date.now()` dans le corps), `trend?: TrendSignal`
(`'down' | 'stable' | 'up' | 'insufficient'`) et `hasGeoLocation?: boolean`
(par défaut `true`). Le module consomme `computeFreshness` (ticket 003) comme
seam officiel de fraîcheur.
